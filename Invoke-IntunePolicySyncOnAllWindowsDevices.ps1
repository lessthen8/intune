...
.EXAMPLE
Just run the script Invoke-IntunePolicySyncOnAllWindowsDevices.ps1 without any parameters, as the user enroling the device.
.COPYRIGHT
...
Start-Transcript "$($env:TEMP)\script_Invoke-IntunePolicySyncOnAllWindowsDevices.ps1_log.txt" -Force
...
if (($deviceObjList).count -gt 0){
    ...
    Write-Output "No Windows devices found in Intune (You might want to verify this manually in the Intune Portal)"
}
